...
wp <- function(team_id, season = SEASONS, exclude = NULL, print_id = TRUE) {
  ...
  games <- WRegularSeasonCompactResults.csv[which(WRegularSeasonCompactResults.csv$Season %in% season),]
...
pct <- function(stat, team_id, season = SEASONS, print_id = TRUE) {
  ...
  games <- WRegularSeasonDetailedResults.csv[which(WRegularSeasonDetailedResults.csv$Season %in% season),]
...
if(TRUE) {
  ...
  RegularSeason <- data.frame(matrix(ncol = 5, 
                               nrow = length(WRegularSeasonDetailedResults.csv[, 1])))
  ...
  RegularSeason$teamA <- c(WRegularSeasonDetailedResults.csv$WTeamID[
      which(WRegularSeasonDetailedResults.csv$WTeamID < WRegularSeasonDetailedResults.csv$LTeamID)],
    WRegularSeasonDetailedResults.csv$LTeamID[
      which(WRegularSeasonDetailedResults.csv$LTeamID < WRegularSeasonDetailedResults.csv$WTeamID)])
  
  RegularSeason$season <- c(WRegularSeasonDetailedResults.csv$Season[
      which(WRegularSeasonDetailedResults.csv$WTeamID < WRegularSeasonDetailedResults.csv$LTeamID)],
    WRegularSeasonDetailedResults.csv$Season[
      which(WRegularSeasonDetailedResults.csv$LTeamID < WRegularSeasonDetailedResults.csv$WTeamID)])
  
  RegularSeason$teamB <- c(WRegularSeasonDetailedResults.csv$LTeamID[
    which(WRegularSeasonDetailedResults.csv$WTeamID < WRegularSeasonDetailedResults.csv$LTeamID)],
    WRegularSeasonDetailedResults.csv$WTeamID[
      which(WRegularSeasonDetailedResults.csv$LTeamID < WRegularSeasonDetailedResults.csv$WTeamID)])
  
  RegularSeason$win <- factor(c(rep('W', length(which(WRegularSeasonDetailedResults.csv$WTeamID < WRegularSeasonDetailedResults.csv$LTeamID))),
                   rep('L', length(which(WRegularSeasonDetailedResults.csv$LTeamID < WRegularSeasonDetailedResults.csv$WTeamID)))),
                   levels = c('L','W'))
  RegularSeason$spread <- c(WRegularSeasonDetailedResults.csv$WScore[
      which(WRegularSeasonDetailedResults.csv$WTeamID < WRegularSeasonDetailedResults.csv$LTeamID)],
    WRegularSeasonDetailedResults.csv$LScore[
      which(WRegularSeasonDetailedResults.csv$LTeamID < WRegularSeasonDetailedResults.csv$WTeamID)]) -
  c(WRegularSeasonDetailedResults.csv$LScore[
      which(WRegularSeasonDetailedResults.csv$WTeamID < WRegularSeasonDetailedResults.csv$LTeamID)],
    WRegularSeasonDetailedResults.csv$WScore[
      which(WRegularSeasonDetailedResults.csv$LTeamID < WRegularSeasonDetailedResults.csv$WTeamID)])
  
  # 
  # RegularSeason$teamApt <- c(WRegularSeasonDetailedResults.csv$Wscore[
  #   which(WRegularSeasonDetailedResults.csv$WTeamID < WRegularSeasonDetailedResults.csv$LTeamID)],
  #   WRegularSeasonDetailedResults.csv$Lscore[
  #     which(WRegularSeasonDetailedResults.csv$LTeamID < WRegularSeasonDetailedResults.csv$WTeamID)])
  # RegularSeason$teamBpt <- c(WRegularSeasonDetailedResults.csv$Lscore[
  #   which(WRegularSeasonDetailedResults.csv$WTeamID < WRegularSeasonDetailedResults.csv$LTeamID)],
  #   WRegularSeasonDetailedResults.csv$Wscore[
  #     which(WRegularSeasonDetailedResults.csv$LTeamID < WRegularSeasonDetailedResults.csv$WTeamID)])
  # # 
  # #new - need to add the dates in to the structure too
  # RegularSeason$Daynum <- c(WRegularSeasonDetailedResults.csv$Daynum[
  #   which(WRegularSeasonDetailedResults.csv$WTeamID < WRegularSeasonDetailedResults.csv$LTeamID)],
  #   WRegularSeasonDetailedResults.csv$Daynum[
  #     which(WRegularSeasonDetailedResults.csv$LTeamID < WRegularSeasonDetailedResults.csv$WTeamID)])
...
if(TRUE) {
  ...
  Tourney <- data.frame(matrix(ncol = 5, 
                                     nrow = length(WNCAATourneyDetailedResults.csv[, 1])))
  ...
  Tourney$teamA <- c(WNCAATourneyDetailedResults.csv$WTeamID[
    which(WNCAATourneyDetailedResults.csv$WTeamID < WNCAATourneyDetailedResults.csv$LTeamID)],
    WNCAATourneyDetailedResults.csv$LTeamID[
      which(WNCAATourneyDetailedResults.csv$LTeamID < WNCAATourneyDetailedResults.csv$WTeamID)])
  
  Tourney$season <- c(WNCAATourneyDetailedResults.csv$Season[
    which(WNCAATourneyDetailedResults.csv$WTeamID < WNCAATourneyDetailedResults.csv$LTeamID)],
    WNCAATourneyDetailedResults.csv$Season[
      which(WNCAATourneyDetailedResults.csv$LTeamID < WNCAATourneyDetailedResults.csv$WTeamID)])
  
  Tourney$teamB <- c(WNCAATourneyDetailedResults.csv$LTeamID[
    which(WNCAATourneyDetailedResults.csv$WTeamID < WNCAATourneyDetailedResults.csv$LTeamID)],
    WNCAATourneyDetailedResults.csv$WTeamID[
      which(WNCAATourneyDetailedResults.csv$LTeamID < WNCAATourneyDetailedResults.csv$WTeamID)])
  
  Tourney$win <- factor(c(rep('W', length(which(WNCAATourneyDetailedResults.csv$WTeamID < WNCAATourneyDetailedResults.csv$LTeamID))),
                                rep('L', length(which(WNCAATourneyDetailedResults.csv$LTeamID < WNCAATourneyDetailedResults.csv$WTeamID)))),
                              levels = c('L','W'))
  Tourney$spread <- c(WNCAATourneyDetailedResults.csv$WScore[
    which(WNCAATourneyDetailedResults.csv$WTeamID < WNCAATourneyDetailedResults.csv$LTeamID)],
    WNCAATourneyDetailedResults.csv$LScore[
      which(WNCAATourneyDetailedResults.csv$LTeamID < WNCAATourneyDetailedResults.csv$WTeamID)]) -
    c(WNCAATourneyDetailedResults.csv$LScore[
      which(WNCAATourneyDetailedResults.csv$WTeamID < WNCAATourneyDetailedResults.csv$LTeamID)],
      WNCAATourneyDetailedResults.csv$WScore[
        which(WNCAATourneyDetailedResults.csv$LTeamID < WNCAATourneyDetailedResults.csv$WTeamID)])
  
  
  # Tourney$teamApt <- c(WNCAATourneyDetailedResults.csv$Wscore[
  #   which(WNCAATourneyDetailedResults.csv$WTeamID < WNCAATourneyDetailedResults.csv$LTeamID)],
  #   WNCAATourneyDetailedResults.csv$Lscore[
  #     which(WNCAATourneyDetailedResults.csv$LTeamID < WNCAATourneyDetailedResults.csv$WTeamID)])
  # Tourney$teamBpt <- c(WNCAATourneyDetailedResults.csv$Lscore[
  #   which(WNCAATourneyDetailedResults.csv$WTeamID < WNCAATourneyDetailedResults.csv$LTeamID)],
  #   WNCAATourneyDetailedResults.csv$Wscore[
  #     which(WNCAATourneyDetailedResults.csv$LTeamID < WNCAATourneyDetailedResults.csv$WTeamID)])
  # 
  ...
  #new - need to add the dates in to the structure too
  # Tourney$Daynum <- c(WNCAATourneyDetailedResults.csv$Daynum[
  #   which(WNCAATourneyDetailedResults.csv$WTeamID < WNCAATourneyDetailedResults.csv$LTeamID)],
  #   WNCAATourneyDetailedResults.csv$Daynum[
  #     which(WNCAATourneyDetailedResults.csv$LTeamID < WNCAATourneyDetailedResults.csv$WTeamID)])
  # 
}
...
#incorporate seeds
RFdata<-merge.data.frame(RFdata, WNCAATourneySeeds.csv, by.x = c("teamA", "season"), 
                            by.y = c("TeamID", "Season"), all.x = TRUE)
RFdata<-merge.data.frame(RFdata, WNCAATourneySeeds.csv, by.x = c("teamB", "season"), 
                            by.y = c("TeamID", "Season"), all.x = TRUE)
...
aucHyperparameters <- apply(searchGridSubCol, 1, function(parameterList){
  ...
  xgboostModelCV <- xgb.cv(data =  xgtrain,
                           label = xgtarget,
                           nrounds = ntrees,
                           nfold = 5,
                           prediction = TRUE,
...
output<-merge.data.frame(output, WNCAATourneySeeds.csv, by.x = c("team1", "season"), 
                            by.y = c("TeamID", "Season"), all.x = TRUE)
output<-merge.data.frame(output, WNCAATourneySeeds.csv, by.x = c("team2", "season"), 
                            by.y = c("TeamID", "Season"), all.x = TRUE)
...
RFoutput<-merge.data.frame(RFoutput, WTeams.csv, by.x = c("team1"), by.y = c("TeamID"))
RFoutput<-merge.data.frame(RFoutput, WTeams.csv, by.x = c("team2"), by.y = c("TeamID"))
...
region <- WNCAATourneySeeds.csv %>% filter(Season == 2022)
...
write.csv(temp, "womenRFourSubmission.csv", row.names = FALSE)
write.csv(RFoutput, "womenRFoutputforwes.csv", row.names = FALSE)

temp <- data.frame(RFoutput$ID, RFoutput$calibprob)
temp <- rename(temp, c("RFoutput.ID" = "ID", "RFoutput.calibprob" = "Pred"))
write.csv(temp, "womenCalRFourSubmission.csv", row.names = FALSE)
write.csv(RFoutput, "womenRFoutputforwesWcal.csv", row.names = FALSE)

temp <- data.frame(RFoutputsub1$ID, RFoutputsub1$winprob)
temp <- rename(temp, c("RFoutputsub1.ID" = "ID", "RFoutputsub1.winprob" = "Pred"))
write.csv(temp, "womenRFourSubmission1.csv", row.names = FALSE)


temp <- data.frame(RFoutputsub2$ID, RFoutputsub2$winprob)
temp <- rename(temp, c("RFoutputsub2.ID" = "ID", "RFoutputsub2.winprob" = "Pred"))
write.csv(temp, "womenRFourSubmission2.csv", row.names = FALSE)
...
#bring in team names too
XGBoutput<-merge.data.frame(XGBoutput, WTeams.csv, by.x = c("team1"), by.y = c("TeamID"))
XGBoutput<-merge.data.frame(XGBoutput, WTeams.csv, by.x = c("team2"), by.y = c("TeamID"))
